...
- **SSD 配置**：
    ...
    ```ini
    # 禁用 SSD 的写入缓存（避免数据丢失风险）
    bluestore_block_wal_mode = 1     # WAL 独占单独设备
    bluestore_cache_autotune = false # 固定缓存大小防止抖动
    ```
    
- **避免过度配置**：
    - 若 `osd_iodepth_threads × bluestore_iodepth` 超过 NVMe SSD 的硬件队列深度，会导致性能下降。
    - 参考公式：线程数 × 单线程队列深度 ≈ NVMe SSD 的硬件队列深度（如 64K）。

---

### **4. 与其他 I/O 机制对比**

#### **(1) libaio vs io_uring**

- **libaio**：
    - 依赖内核接口 `io_submit/io_getevents`，适用简单异步场景，但对高并发支持有限。
    - BlueStore 默认选项，稳定性高。
- **io_uring**：
    - 更新且高效的异步框架，支持批量和轮询模式，减少上下文切换。
    - 在 Ceph Quincy+ 中实验性支持（需配置 `bluestore_ioring = true`）。

#### **(2) 同步 I/O（不推荐）**

- 绕过 AIO 直接使用 `sync` 写入（如 `bluestore_aio = false`），会导致吞吐量暴跌，仅用于调试。

---

### **总结**

Ceph 通过 **BlueStore 的 libaio + 多线程池** 机制实现异步 I/O：

1. **多线程并行**：通过 `osd_iodepth_threads` 扩展并发通道。
2. **深度队列支持**：`bluestore_iodepth` 填充 NVMe SSD 硬件队列。
3. **事件驱动回调**：`io_getevents` 监听完成事件，减少阻塞。

优化关键：**根据 NVMe SSD 的硬件队列深度**，调整 `osd_iodepth_threads` 和 `bluestore_iodepth` 的乘积，使其接近 SSD 的物理极限，同时避免过度配置导致竞争。