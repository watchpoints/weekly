...
内存管理(看到这四个单词闹钟一片空白，高大上其实做法很简单)

隔离 和删除。

- Redis主要通过控制内存上限和回收策略实现内存管

  问题：如何一台机器上部署多个实例（容器思想，简单内存上线，容器做准备）

 ![防止被oom](image/image-20210322133437775.png)
...
- redis如何扩容。
  ...
  ![image-20210322133737212](image/image-20210322133737212.png)

> Redis默认无限使用服务器内存，为防止极端情况下导致系统内存耗 尽，
>
> 建议所有的Redis进程都要配置maxmemory。
>
> 在保证物理内存可用的情况下，系统中所有Redis实例可以调整 maxmemory参数来
>
> 达到自由伸缩内存的目的。
...
- 如果没有过期key怎办，我业务不使用（其中一个策略）
- 从伪代码可以看到，频繁执行回收内存成本很高，主要包括查找可回收 键和删除键的开销



![image-20210322135118381](image/image-20210322135118381.png)
